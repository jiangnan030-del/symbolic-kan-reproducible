...
def rank_primitive_candidates(
    ...
    *,
    ...
    The score combines deterministic gate probability with a small, explicit
    complexity prior. It is evidence from this model only; it is not an R² score
    and does not certify that the selected primitive is the true governing law.
    """
...
def prune_edges(
    ...
    *,
    ...
    Low-confidence selections are retained rather than silently deleting an entire
    unit. They are marked in the returned report so callers can reject, revisit, or
    compare them across seeds.
    """
...
def write_structure_svg(
    ...
    *,
    ...
    output_positions = _vertical_positions(1, height)
    ...
    for block_index, block_data in enumerate(structure["blocks"]):
        ...
        for unit_index, unit in enumerate(block_data["units"]):
            ...
            for source_index, weight in enumerate(weights[: len(previous_y)]):
                ...
                if magnitude < 1e-6:
                    continue
                opacity = 0.22 + 0.68 * magnitude
                stroke_width = 1.0 + 2.8 * magnitude
                lines.append(
                    ...
                    f'opacity="{opacity:.2f}" fill="none"/>'
                )
    ...
    for y_value in previous_y:
        lines.append(
            ...
            f'C {source_x + 90:.1f} {y_value:.1f}, '
            f'{target_x - 90:.1f} {output_positions[0]:.1f}, '
            f'{target_x - 30:.1f} {output_positions[0]:.1f}" '
    ...
    for block_index, block_data in enumerate(structure["blocks"]):
        ...
        for unit_index, unit in enumerate(block_data["units"]):
            ...
            nodes.append(
                ...
                f'b{block_index} · u{unit_index}</text></g>'
            )

    nodes.append(
        f'<rect x="{x_positions[-1] - 30:.1f}" '
        f'y="{output_positions[0] - 30:.1f}" width="60" height="60" '
        'rx="14" fill="#2783DE"/>'
        f'<text x="{x_positions[-1]:.1f}" y="{output_positions[0] + 6:.1f}" '
...
def build_symbolic_report(
    ...
    *,
    ...
    return {
        ...
        "expression": export_expression(model, variables=variable_names),
        "history": history or [],
...
<style>
:root {{ color-scheme: light; --text:#2C2C2B; --muted:#7D7A75; --border:#E6E5E3; --soft:#F9F8F7; --blue:#2783DE; }}
...
def write_symbolic_report(
    ...
    *,
    ...
    """Write JSON, text, SVG, and self-contained audit-report entry points."""
    ...
    report_path = output / "symbolic_report.json"
    expression_path = output / "expression.txt"
    svg_path = output / "structure.svg"
    html_path = output / "report.html"
    report_path.write_text(
        json.dumps(report, indent=2, default=str), encoding="utf-8"
    )
    expression_path.write_text(str(report["expression"]), encoding="utf-8")
    write_structure_svg(model, svg_path)
